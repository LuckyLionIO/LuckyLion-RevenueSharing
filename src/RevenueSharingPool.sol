...
contract RevenueSharingPool is Ownable {
   ...
   function removeStakeholder(address account) internal {
    ...
    function updatePoolInfo(uint256 winLoss, uint256 TPV, uint256 buyBack, uint256 percentOfRevshare, uint256 roundID) internal {
   ...
   function isStakeholder(address _address) public view returns(bool, uint256) {
    ...
    function calMinAmountOut(uint256 amountIn, uint256 percentOfSlippage, address[] memory path) public view returns (uint256) {
    ...
    function depositRevenue(
        ...
        uint256 minOutputAmount = calMinAmountOut(BUSDBalance, percentOfSlippage, BUSDToOutputPath);
    ...
    function _swapExactNativeForBUSD(uint256 amount, uint256 percentOfSlippage) private {
        ...
        uint256 minOutputAmount = calMinAmountOut(amount, percentOfSlippage, path);
    ...
    function _swapTokensForBUSD(InputToken calldata inputTokens, uint256 percentOfSlippage) private {
        if (inputTokens.token != address(BUSD) && inputTokens.tokenToBUSDPath[inputTokens.tokenToBUSDPath.length - 1] == address(BUSD)) {
            IERC20(inputTokens.token).approve(address(exchangeRouter), MAX_NUMBER);
            uint256 minOutputAmount = calMinAmountOut(inputTokens.amount, percentOfSlippage, inputTokens.tokenToBUSDPath);
            exchangeRouter.swapExactTokensForTokens(
                inputTokens.amount,
                minOutputAmount,
    ...
    function _swapBUSDForToken(uint256 amount, uint256 percentOfSlippage, address[] calldata path)
        private
        returns (uint256)
    {
