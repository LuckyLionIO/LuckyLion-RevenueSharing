...
contract RevenueSharingPool_Test is Ownable {
    // Utility Libraries
    ...
    function updatePoolInfo(
        ...
        _poolInfo.totalLuckyRevenue = totalLuckyRevenue[roundID];
    }
    ...
    function calculateLuckyReward(address account, uint256 roundId)
        internal
        view
        returns (uint256)
    {
    ...
    function getStakeAmount(uint256 roundId, uint256 day)
        external
        view
        returns (uint256)
    {
    ...
    function getDepositDate() internal view returns (uint256) {
        for (uint256 i = 1; i <= MAX_DATE; i++) {
            if (
                block.timestamp >= START_ROUND_DATE &&
                block.timestamp < START_ROUND_DATE + (i * 1 days)
            ) {
                return i;
            }
        }
    ...
    function getTotalLuckyRewardPerDay(uint256 roundId)
        public
        view
        returns (uint256)
    {
    ...
    // (RevPool)
    function getLuckyRewardPerRound(uint256 roundId)
        external
        view
        returns (uint256)
    {
    ...
    function isStakeUpToDate(uint256 currentRoundId)
        internal
        view
        returns (bool)
    {
    ...
    // This function is for testing only! (must be remove when deploy to mainnet)
    function testDepositRevenue(
        uint256 revenueAmount,
        uint256 winLoss,
        uint256 percentOfRevshare,
        uint256 TPV
    ) external isWhitelisted(msg.sender) {
        lucky.transferFrom(msg.sender, address(this), revenueAmount);
        uint256 roundId = getCurrentRoundId();
        updatePoolInfo(winLoss, TPV, revenueAmount, percentOfRevshare, roundId); // update round pool info
        totalLuckyRevenue[roundId] += revenueAmount;
        // START_ROUND_DATE = block.timestamp;
        // updateRoundId();
        // uint256 currentRoundId = getCurrentRoundId();
    ...
    // This function is for testing only! (must be remove when deploy to mainnet)
    function updatePool() external isWhitelisted(msg.sender) {
        START_ROUND_DATE = block.timestamp;
        updateRoundId();
        uint256 currentRoundId = getCurrentRoundId();
        updateTotalStake(currentRoundId); // update new round total stake
    ...
    function depositRevenue(
        ...
        if (msg.value > 0) {
            ...
            _swapExactNativeForBUSD(wNativeBalance, percentOfSlippage);
        }

        if (inputTokens.length > 0) {
            for (uint256 i; i < inputTokens.length; i++) {
                if (inputTokens[i].token == address(lucky)) {
                    // if token is LUCKY just transfer to contract directly
                    IERC20(lucky).safeTransferFrom(
                        msg.sender,
                        address(this),
                    ...
                    // if token is BUSD just transfer to contract first
                    ...
                    // if token is not both LUCKY or BUSD let's swap it's to BUSD first
                    _transferTokensToCave(inputTokens[i]);
                    _swapTokensForBUSD(inputTokens[i], percentOfSlippage);
                }
            }
        }

        uint256 BUSDBalance = BUSD.balanceOf(address(this));
        uint256 minOutputAmount = calMinAmountOut(
            BUSDBalance,
            percentOfSlippage,
            BUSDToOutputPath
        );
        ...
        updatePoolInfo(winLoss, TPV, BUSDBalance, percentOfRevshare, roundId); // update round pool info
        START_ROUND_DATE = block.timestamp;
        updateRoundId();
        uint256 currentRoundId = getCurrentRoundId();
        updateTotalStake(currentRoundId); // update new round total stake
    ...
    function _swapExactNativeForBUSD(uint256 amount, uint256 percentOfSlippage)
        private
    {
    ...
    function _swapTokensForBUSD(
        ...
        if (
            inputTokens.token != address(BUSD) &&
            inputTokens.tokenToBUSDPath[
                inputTokens.tokenToBUSDPath.length - 1
            ] ==
            address(BUSD)
        ) {
            IERC20(inputTokens.token).approve(
    ...
    function _swapBUSDForToken(
        ...
        require(path[0] == address(BUSD), "tokenInput is not BUSD");
