...
contract MockRevenueSharingPool is Ownable {
    // Utility Libraries  
    ...
    event DepositStake(address indexed account, uint256 amount);
    event WithdrawStake(address indexed account, uint256 amount);
    ...
    function depositToken(uint256 amount) external {
        ...
        emit DepositStake(msg.sender, amount);
    }
    ...
    function claimReward() external {
        ...
        emit ClaimReward(msg.sender, claimableLuckyReward);
    }
   ...
   function removeStakeholder(address account) internal {
    ...
    function updatePoolInfo(uint256 winLoss, uint256 TPV, string memory symbol, uint256 revenueAmount, uint256 percentOfRevshare, uint256 roundID) public isWhitelisted(msg.sender) {
   ...
   function isStakeholder(address _address) public view returns(bool, uint256) {
    ...
    function depositRevenue(uint256 amount) external isWhitelisted(msg.sender) {
        uint256 roundId = getCurrentRoundId();
